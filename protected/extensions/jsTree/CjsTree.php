...
class CJsTree extends CWidget
{
    ...
    public function init()
    {
        ...
        $cs->registerScriptFile($this->baseUrl.'/jquery.hotkeys.js');
        ...
        {
            $c .= ".bind('$key.jstree', ".CJavaScript::encode($val).")";
        }
        //$cs->registerScript('Yii.CJsTree#'.$id,"$(function () { $(\"#{$id}\").tree($options); });");
        $cs->registerScript('Yii.CJsTree#'.$id,"$(\"#{$id}\").jstree($options)$c;");
	...
	{ 
		...
		//$this->body=$this->body.self::saveDataAsHtml($this->data);
                $this->body=$this->body.self::saveDataAsJson($this->json_data);
		$this->body=$this->body."\n</ul>";
	...
	protected function getClientOptions()
	{
        ...
        $optionsParams=array('json_data','selected','opened','languages','path','cookies','ui','callback','rules','lang','plugins');
		...
		return CMap::mergeArray($options, $res);
	}
